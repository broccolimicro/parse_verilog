...
R"(module assignment_test(
	...
	if (dut.items.size() == 1) {
		...
		EXPECT_EQ(assign_stmt->assign.left.to_string(), "c");
...
R"(module loop_statement_test(
	...
	if (dut.items.size() == 1) {
		...
		if (always_block->body.sub.size() > 0) {
			...
			EXPECT_EQ(for_ptr->init.left.to_string(), "i");
			
			// Check that step is "i = i + 1"
			EXPECT_EQ(for_ptr->step.left.to_string(), "i");
		}
	}
